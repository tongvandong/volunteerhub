    ...
    <div key={comment.id} className={`flex gap-2 group ${isReply ? 'ml-9' : ''}`}>
      ...
      <div className="min-w-0 flex-1">
        <div className="rounded-xl border border-gray-100 bg-white px-3 py-2 shadow-sm">
          <p className="text-xs font-semibold text-gray-800">{comment.author?.name || comment.author?.userName || 'Nguoi dung'}</p>
          <p className="mt-0.5 whitespace-pre-wrap text-sm text-gray-700">{renderMentionText(comment.content)}</p>
        </div>
        ...
        <form onSubmit={submit} className="flex gap-2">
          ...
          <div className="flex flex-1 gap-2">
            <input
              ...
              className="flex-1 rounded-full border border-gray-200 bg-white px-3 py-1.5 text-sm focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500"
            />
            <button type="submit" disabled={submitting || !text.trim()} className="text-primary-600 hover:text-primary-700 disabled:text-gray-300">
              <i className="fa-solid fa-paper-plane" />
            </button>
          </div>
        </form>
        ...
        {(poll.options || []).map((option) => {
          ...
          return (
            <button
              ...
              disabled={expired}
              className={`relative w-full overflow-hidden rounded-lg border p-2 text-left text-sm ${voted ? 'border-primary-400 bg-primary-50' : 'border-gray-200 bg-white hover:bg-gray-50'} disabled:cursor-not-allowed`}
            >
              <div className="absolute inset-y-0 left-0 bg-primary-100" style={{ width: `${percent}%` }} />
              <div className="relative flex items-center justify-between gap-3">
                <span>{option.text}</span>
                <span className="text-xs text-gray-500">{percent}% ({option.voteCount || 0})</span>
              </div>
            </button>
          );
        })}
  ...
  return (
    <div className="card overflow-hidden">
      ...
      <div className="flex items-start gap-3 p-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            <p className="text-sm font-semibold text-gray-900">{post.author?.name || post.author?.userName || 'Nguoi dung'}</p>
            <div className="flex items-center gap-2">
              ...
              {canDelete && (
                <button onClick={() => onDelete(post.id)} className="text-gray-400 transition-colors hover:text-red-500">
      ...
      <div className="px-4 pb-3">
        ...
        <p className="whitespace-pre-wrap text-sm text-gray-800">{renderMentionText(post.content)}</p>
        {post.imageUrl && <img src={post.imageUrl} alt="" className="mt-3 max-h-80 w-full rounded-lg border border-gray-100 object-cover" />}
        {post.attachmentUrl && (
      ...
      <div className="flex items-center gap-4 border-t border-gray-100 px-4 py-2.5">
        ...
        <button onClick={() => setShowComments(!showComments)} className="flex items-center gap-1.5 text-sm text-gray-400 transition-colors hover:text-primary-600">
...
export default function Channel() {
  ...
  useEffect(() => {
    ...
    const connection = new signalR.HubConnectionBuilder()
      .withUrl('http://localhost:5003/hubs/channel', { accessTokenFactory: () => token })
        ...
        <div className="card p-3">
          ...
          {(channel?.subChannels || []).map((sub) => (
            <Link key={sub.id} to={`/channels/${sub.id}`} className={`mt-1 flex items-center gap-2 rounded-lg px-2 py-2 text-sm hover:bg-gray-100 ${Number(id) === sub.id ? 'bg-primary-50 text-primary-700' : ''}`}>
              <i className="fa-solid fa-clock text-gray-500" /> {sub.shift?.name || sub.name}
            </Link>
          ))}
        </div>
      </aside>
