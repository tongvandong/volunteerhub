...
namespace BaseCore.Services.Authen
{
    public interface IUserService
    {
        ...
        Task<User?> AuthenticateByUsernameOrEmail(string identifier, string password);
    ...
    public class UserService : IUserService
    {
        ...
        public async Task<User?> AuthenticateByUsernameOrEmail(string identifier, string password)
        {
            if (string.IsNullOrWhiteSpace(identifier) || string.IsNullOrWhiteSpace(password))
                return null;

            var user = await _userRepository.GetByUsernameAsync(identifier.Trim());

            if (user == null && identifier.Contains('@'))
            {
                user = await _userRepository.GetByEmailAsync(identifier.Trim());
            }
